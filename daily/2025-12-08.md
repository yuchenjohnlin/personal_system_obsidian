...
- Added User schema, model, user create api
- added login api (no authentication)
	- so that I can test the backend from the frontend
	- installed sqlite browser to look at the database
- Added register page - to add username into the database
- Tested out that the register and login works 
...
### User Schema Design 
Besides the username password_hash and email. The user schema is defined to protect the user. 

**will there actually be people who try multiple passwords to break into someone's account ? 

Yes—credential‑stuffing and password spraying are common. Attackers use breached password lists to try one or few passwords across many accounts, or many passwords against a single account. Practical defenses to add:

- Rate limiting and progressive lockout on failed logins; store failed_login_attempts + locked_until.
- Enforce strong password hashing (bcrypt/argon2) and require password resets after suspected compromise; track password_changed_at.
- Multi‑factor authentication and login notifications.
- IP/device anomaly detection; throttle or challenge suspicious attempts (CAPTCHA/OTP).
- Unique constraints on identifiers and logging of last_login_at/last_login_ip for audit.
